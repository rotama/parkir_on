		...
		<tr>
			<td>
				<pre>
					...
					Tanggal Keluar				: @if(count($keluar) > 0){{ $masterbookings->tgl_keluar }}@else-@endif<br>
					Status												: {{ $masterbookings->status }}<br>
					Harga Denda 						: @if(count($keluar) > 0)Rp.{{ number_format($hrg_denda,2) }}@else-@endif <br>
					Keterlambatan 				: @if(count($keluar) > 0 AND $keluar->keterlambatan > 0)<font color="red">{{ $keluar->keterlambatan }} hari </font>@else 0 hari @endif<br>
					Kena Biaya Denda 	: @if(count($ada_denda) > 0 AND $keluar->keterlambatan > 0)<font color="red">{{ number_format($denda,2) }}</font>@else 0 @endif<br>
